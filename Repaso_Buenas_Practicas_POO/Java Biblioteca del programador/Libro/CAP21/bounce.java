...
class BounceImage
  {
    ...
    public BounceImage(bounce parent, int index)
      {
        this.parent = parent;
        bounce_index = index;
    ...
    public void paint(Graphics g)
      {
        ...
        if (parent.bounceimages[bounce_index][i] == null)
          i = 0;
        ...
        g.drawImage(parent.bounceimages[bounce_index][i],
                    (int) x, (int) y, null);
    ...
    public void step(long deltaT)
      {
        ...
        while (findex <= 0.0)
          findex += parent.bounceimages[bounce_index].length;

        index = ((int) findex) %
                parent.bounceimages[bounce_index].length;
      }
  }
    ...
    void makeImages()
      {
        ...
        int items = 1;
        parameter = getParameter("ITEMS");
        if (parameter != null)
          items = Integer.parseInt(parameter);

        bounceimages = new Image[items][];

        for (int item = 0; item < items; item++)
          {
            bounceimages[item] = new Image[8];
            String dir_name = getParameter("ITEM_" + (item+1));
            for (int i = 1 ; i <= 8 ; i++)
              bounceimages[item][i-1] = getImage(getCodeBase(),
                            "images/" + dir_name + "/t" + i + ".gif");
          }

        images = new BounceImage[items];
        for (int i = 0; i < items; i++)
          {
            BounceImage img = images[i] = new BounceImage(this, i);
