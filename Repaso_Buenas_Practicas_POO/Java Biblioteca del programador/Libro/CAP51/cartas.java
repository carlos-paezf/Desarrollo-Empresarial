...
public class cartas
  {
    ...
    public void dibujar(int carta, int x, int y, int w, int h)
      {
        ...
        g.drawRoundRect(x, y, w, h, w/20, h/20);
    ...
    public void dibujar_reverso(int x, int y, int w, int h)
      {
        g.setColor(Color.white);
        g.fillRoundRect(x, y, w, h, w/20, h/20);
        ...
        for (int i = x + x_inc; i < x_fin; i += x_inc)
          g.drawLine(i, y, i, y_fin);

        for (int j = y + y_inc; j < y_fin; j += y_inc)
          g.drawLine(x, j, x_fin, j);
      }
